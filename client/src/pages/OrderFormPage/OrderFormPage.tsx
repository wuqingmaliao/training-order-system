...
const OrderFormPage = () => {
  ...
  useEffect(() => {
    ...
    if (isEdit && id) {
      trainingOrders.getOrderDetail(id).then(order => {
        form.reset({
          trainingType: order.trainingType,
          ...
          promisedStudent: order.promisedStudent || '无',
  ...
  const onSubmit = async (data: OrderFormData) => {
    ...
    try {
      if (isEdit && id) {
        await trainingOrders.updateOrder(id, {
          ...data,
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <Card className="w-full max-w-md shadow-lg">
          <CardContent className="pt-8 pb-8 flex flex-col items-center text-center">
            ...
            <p className="text-muted-foreground mb-1">订单已保存</p>
            <p className="text-sm text-muted-foreground mb-6">
              订单编号：<span className="font-mono text-foreground">{orderNo}</span>
            </p>
            <div className="flex gap-3">
              <Button onClick={handleReset} variant="outline">继续填写</Button>
              <Button onClick={() => navigate('/my-orders')}>查看我的订单</Button>
            </div>
          </CardContent>
        </Card>
      </div>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="font-semibold">学员信息</h3>
                  </div>
                  <div className="space-y-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>对应学员姓名 <span className="text-destructive">*</span></FormLabel>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl><Input placeholder="请输入手机号码" {...field} /></FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      />
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl><Input placeholder="请输入转介绍人" {...field} /></FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      />
                ...
                <div>
                  <div className="flex items-center gap-2 mb-4">
                    <div className="w-1 h-5 bg-primary rounded-full" />
                    <h3 className="font-semibold">业务信息</h3>
                  </div>
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <Select onValueChange={field.onChange} value={field.value}>
                              <FormControl><SelectTrigger><SelectValue placeholder="请选择培训类型" /></SelectTrigger></FormControl>
                              <SelectContent>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <Select onValueChange={field.onChange} value={field.value}>
                              <FormControl><SelectTrigger><SelectValue placeholder="请选择客户来源" /></SelectTrigger></FormControl>
                              <SelectContent>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <Select onValueChange={field.onChange} value={field.value}>
                              <FormControl><SelectTrigger><SelectValue placeholder="请选择报考项目" /></SelectTrigger></FormControl>
                              <SelectContent>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl><Input placeholder="请输入负责人" {...field} /></FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                      { name: 'originalPrice', label: '原价（元）' },
                      { name: 'actualPayment', label: '实收金额（元）' },
                      { name: 'discountedPrice', label: '折后金额（元）' },
                      { name: 'remainingAmount', label: '尾款（元）' },
                    ].map(f => (
                      <FormField
                        key={f.name}
                ...
                <div className="pt-4 flex justify-center gap-3">
                  ...
                  <Button type="submit" size="lg" disabled={submitting}>
                    {submitting ? '提交中...' : isEdit ? '保存修改' : '提交订单'}
                  </Button>
                </div>
              </form>
            </Form>
